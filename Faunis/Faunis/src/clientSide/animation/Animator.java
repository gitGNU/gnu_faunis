...
public class Animator<ANIMATEABLE extends Animateable> implements ModTimer {
	...
	public Animator(AnimatorModule<ANIMATEABLE, ?, ?> parent,
			ANIMATEABLE animateable, long delay, AniEndType endType,
			int maxFrameIndex) {
		this.parent = parent;
	...
	/**
	 * Starts the Animator. Has to be explicitely called, as that isn't done
	 * automatically.<br />
	...
	/**
	 * asserts that at the method end, no timerTask is running.
	 */
	@Override
	public void stop() {
		synchronized (runningMutexKey) {
		...
		protected void animate() {
			synchronized (runningMutexKey) {
				...
				case repeat:
					...
					Logger.log("Increase frame from " + oldFrame + " to "
							+ frame);
				...
				case end:
					if (frame < maxFrameIndex) {
						Logger.log("Increase frame from " + frame + " to "
								+ (frame + 1));
