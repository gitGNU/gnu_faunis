...
public class GameWindow extends GraphWin {
	...
	private void loadGrassBackground() {
		...
		BufferedImage grass;
		try {
			grass = ImageIO.read(new File(settings.floorGraphicsPath()+"grass/grass.png"));
		} catch (IOException e) {
			e.printStackTrace();
			throw new RuntimeException("Couldn't load the grass background!");
		}
		float scaleFactor = settings.fieldHeight() / (float) grass.getHeight();
		BufferedImage scaledGrass = ImageScaler.downscale(grass, scaleFactor);
		grassBackground = new BufferedImage(this.getWidth(), this.getHeight(), BufferedImage.TYPE_INT_RGB);
		Graphics graphics = grassBackground.getGraphics();
		for (int y = 0; y < 18; y++) {
			for (int x = 0; x < 20; x++) {
				graphics.drawImage(scaledGrass, x*settings.fieldWidth(), y*settings.fieldHeight(), null);
			}
		}
	}
	...
	public void draw() {
		...
		switch (status) {
			...
			case noCharLoaded:
				graph.drawImage(
					graphicsContentManager.getGUIGraphics("fullscreen_title"), 0, 0, null
				);
				break;
			case exploring:
			case fighting:
				// hack - draw grass background:
				graph.drawImage(grassBackground, 0, 0, null);
				// draw grid:
				drawFieldGrid();
	...
	private void drawFieldGrid() {
		graph.setColor(parent.getClientSettings().gridColor());
